...
**Data:** 19/09/2026; atualizada em 20/09/2026 com Voice & Accountability (WGI) no segundo estágio. Laboratório final em 21/09; apresentação de 20 min em 28/09; manuscrito Qualis até o lançamento das notas.
**Origem das mudanças:** `documentos/analise_critica.md` (crítica da v1, da orientação do professor e da v2).
...
| Zeros | Shift em 1/10/100 mi | Shift de **US\$ 0,5 mi** (metade do menor positivo); amostra sem zeros e shift de 1 mi como robustez; eficiência de escala só sem zeros; dummy `zero_inv` no 2º estágio | Os shifts da v2 iam até a mediana da amostra; a eficiência de escala é indefinida para os zeros sob CCR |
...
- **DMU:** país-ano; **fronteira agrupada** (208), como no template da aula; orientação a produto; escores reportados como 1/eff em (0, 1].
...
- **Robustez do núcleo (todas no script, tabela `t07_robustez.csv`):** forma intensiva; fronteiras anuais; shift de US\$ 1 mi; sem os 17 zeros; T só até 2019; sem Luxemburgo e Singapura; insumos defasados em t−1.
- **Outliers:** supereficiência de Andersen-Petersen (VRS e CRS) e fronteiras parciais (Order-m, Order-α).
- **Segundo estágio:** KS e Kruskal-Wallis por renda, região, `zero_inv` e manufatura acima/abaixo da mediana; Tobit (`censReg`, censura em 0 e 1) e OLS com erros agrupados por país sobre BCC_S, BCC_T, BCC_C, SE e as versões intensivas, com contextuais manufatura % PIB, manufaturados % exportações, governança, Voice & Accountability, ln PIB pc, comércio, Z-score, NPL, `zero_inv` e tendência; sensibilidade ao conjunto de contextuais institucionais e à amostra sem os países de patente ≈ 0 (`t19`); Simar-Wilson Algoritmo 2 com `rDEA::dea.env.robust` (linear em δ, como na aula) e com regressão truncada em **log(δ)** (implementação própria com `truncreg`), necessária porque δ chega a 1.429 em T.
...
|---|---|---|---|
| 1–2 | Preparação, descritivas | `material_disciplina/lesson1.4.r` | `base_v3.csv`, `t01`, `fig01` |
...
| 13 | KS/KW, Tobit, OLS-cluster, Simar-Wilson alg. 2 | `ks.test`, `kruskal.test`, `censReg`, `rDEA::dea.env.robust`, `truncreg` | `t16`–`t18`, `fig10`, `fig11` |

Como rodar: `Rscript scripts/pipeline_v3.R` (≈ 50 s) ou `RAPIDO=FALSE Rscript scripts/pipeline_v3.R` (valores finais; alguns minutos). Contextuais: `bash dados/baixar_wdi.sh`. Pacotes: `Benchmarking`, `rDEA`, `nonparaeff`, `poLCA`, `censReg`, `topsis`, `truncreg`, `tidyverse`, `sandwich`, `lmtest`.
...
3. A fronteira agrupada com contagens resolve o artefato de Luxemburgo/Singapura da v2, mas cria o de insumo minúsculo (Peru, Ucrânia, Malásia, Romênia): supereficiência, Order-m e a robustez sem zeros são obrigatórias no texto.
...
3. Unidade de `AI.Investment` (corrente × constante), origem dos zeros e cobertura por país do rastreador de investimento.
...
5. Robustez "S só com P&D" e "T só com investimento" (insumos específicos a cada produto), para responder à leitura crítica de §6.1.
6. Order-m sobre a forma intensiva (pares comparáveis por tamanho) e supereficiência por modelo.
7. Efeitos de país são inviáveis (96–99 % da variância das contextuais é entre países, inclusive `voice`); manter erros agrupados e dummies de ano; discutir como limitação.
8. Painel: Malmquist só no subpainel balanceado; o adjacente sem balanceamento (154 pares) fica como complemento.
...
| 3 | Diagnóstico de dados: zeros, truncamento das patentes, 37 países × 9 anos (1 min) | `documentos/analise_critica.md` §3.3 |
...
| 9 | Malmquist e fusões regionais; TOPSIS (2 min) | `fig08`, `t13`, `t14`, `t15` |
| 10 | Segundo estágio: testes, Tobit, Simar-Wilson (3 min) | `fig10`, `t16`–`t18` |
| 11 | Diagnóstico setorial: quem converte, quem não, papel da base industrial; Brasil (2 min) | `fig11`, §6.6 |
...
3. Método — DEA agrupado (CCR/BCC), modelos S/T/ST/C, bootstrap, Order-m/α, SFA, segundo estágio (Tobit; Simar-Wilson alg. 2 em log δ), robustez (Fig. 1: fluxograma = tabela de §5).
4. Dados — variáveis e unidades, merge WDI, tratamento dos zeros, truncamento das patentes.
5. Resultados — fronteiras, escores, benchmarks, robustez, dinâmica (Malmquist).
6. Determinantes — H1/H2 nos três estimadores; institucionais e financeiras.
7. Discussão — implicações para países que publicam e não patenteiam; o caso brasileiro; limites da variável de patentes.
8. Limitações e agenda — separabilidade, DEA em rede, order-m condicional, classes latentes em SFA.
...
| Dia | Tarefa |
|---|---|
| 20/09 (dom) | Voice & Accountability incorporada (feito). Confirmar definições na fonte (patentes, investimento, zeros); procurar patentes de IA por país do inventor (OECD.AI, WIPO); rodar `RAPIDO=FALSE`; acrescentar robustez "S só com P&D" |
| 21/09 (lab) | Levar ao professor três decisões: (a) extensiva como principal; (b) o que fazer com os 13 países de patente ≈ 0 e com a colinearidade manufatura × voice (−0,60); (c) alg. 2 em log(δ) para T e C. Ajustar conforme a orientação |
| 22–24/09 | Deck (12 slides) a partir de `resultados/figuras/` e `resultados/`; texto dos slides 10–11 |
...
`resultados/`: `base_v3.csv` (base tratada), `t01`–`t19` (tabelas numeradas como em §5; `t19` = sensibilidade do segundo estágio), `resumo_v3.txt` (log com os números citados), `log_erros.txt` (vazio nesta rodada).
`resultados/figuras/`: `fig01_descritivas`, `fig02_fronteira_2d`, `fig03_densidades_bcc`, `fig04_boxplot_metodos`, `fig05_bootstrap`, `fig06_orderm_alpha`, `fig07_sfa_dea`, `fig08_malmquist`, `fig09_classes_latentes`, `fig10_segundo_estagio`, `fig11_conversao_pais`.
`dados/`: `wdi_contextuais.csv`, `baixar_wdi.sh` (regenera o CSV; o JSON bruto fica fora do git). Raiz: `dados/wgi.voice_accountability.csv` (Voice & Accountability, WGI).
