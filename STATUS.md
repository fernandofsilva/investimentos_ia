...
Atualizado em 20/09/2026. Este arquivo resume o estado da pesquisa: pergunta, base, o que foi testado em cada versão, o que mudou e por quê, os resultados atuais, as decisões tomadas e o que ainda está em aberto. Os documentos completos estão em `documentos/`; os números citados aqui saem de `scripts/pipeline_v3.R` (rodada rápida, `RAPIDO=TRUE`).
...
- **DMU e fronteira.** País-ano; fronteira única com as 208 observações; orientação a produto; escores reportados como 1/eff em (0, 1].
...
| Dinâmica | Respondida: fronteira de publicações por dólar recuou; catch-up positivo |
...
| Deslocamento de US$ 0,5 mi nos zeros | Metade do menor positivo; o BCC-produto é invariante a translações do insumo; SE reportada só sem zeros |
...
3. **Investimento.** Confirmar moeda (corrente × constante), origem dos zeros e cobertura por país do rastreador.
...
|---|---|---|
| `Rscript scripts/pipeline_v3.R` | Roda o template inteiro; grava `resultados/t01`–`t19`, `resultados/figuras/fig01`–`fig11`, `resultados/resumo_v3.txt` | ≈ 50 s |
...
- O trabalho correlato do grupo do professor com o mesmo período e variáveis afins é Fukuyama, Tan & Wanke (2025). O professor mencionou ainda um estudo econométrico (mediação/moderação) sobre esta base, não localizado.
- Referências metodológicas centrais: Ali & Seiford (1990) e Pastor (1996) para invariância à translação; Simar & Wilson (1998, 2007); Daraio, Simar & Wilson (2018); Wilson (1993) e Andersen & Petersen (1993); Olesen, Petersen & Podinovski (2015, 2017) para razões no DEA; Wang & Huang (2007), Guan & Chen (2012) e Kontolaimou et al. (2016) para eficiência de sistemas nacionais de inovação.
...
```
dados/                 base principal, contextuais do WDI e do WGI, script de download
scripts/               pipeline_v3.R (análise completa), replica_analise_critica.R
scripts/relatorio/     modelo do relatório, trechos de R (00_setup, E01–E10), run_prints.R, build_report.py
resultados/            tabelas t01–t19, base tratada, log de execução, prints/, figuras/, escores da crítica
documentos/            relatorio_professor.md, proposta_v3.md, analise_critica.md; anteriores/ com v1 e v2
